<?php
	...
	if ( post_password_required() ) { ?>
		<p class="nocomments">Esta postagem está protegida por senha. Digite a senha para ver os comentários.</p>
	<?php
		return;
	}
   ...
   <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
	...
	<div class = "right-part">
		<div class ="comment-meta">
			...
			<span class = "commentsDate"> <?php printf('%1$s às %2$s', get_comment_date(),  get_comment_time()) ?><?php edit_comment_link('(Editar)','  ','') ?>
	...
	<?php if ($comment->comment_approved == '0') : ?>
	 <em><?php echo 'Seu comentário aguarda moderação.' ?></em>
	 <br />
	<?php endif; ?>
...
<!-- You can start editing here. -->
	<?php 
		...
		$cancel_reply_link = esc_html__('Cancelar resposta','amory');  					
		$title_reply_to = esc_html__('Deixe uma resposta para','amory'); 			
		$label_submit = esc_html__('Deixe um comentário','amory');  		
		$translation_comment_website = esc_html__('Website','amory');  	
		$translation_comment_required = esc_html__('obrigatorio','amory');  
		$translation_comment_mail = esc_html__('E-mail','amory'); 	
		$translation_comment_name = esc_html__('Nome','amory');		
		$translation_comment_closed = esc_html__('Comentários estão fechados.','amory');		
	?>
...
<?php $aria_req = ''; $post_id = ''; $defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
    ...
    'must_log_in' => '',
    'logged_in_as' => '
<p class="logged-in-as">' . sprintf(  wp_kses( __('logado como <a href="%s">%s</a>. <a title="sair desta conta" href="%s">Sair?</a>
' ,'amory'), array(  'a' => array( 'href' => array() ) ) ) .'</p>', esc_url(admin_url( 'profile.php' )), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ),
